...
static char *TAG = "co5300_mipi_test";
...
static void test_init_lcd(void)
{
    ...
    TEST_ESP_OK(esp_lcd_new_panel_co5300(mipi_dbi_io, &panel_config, &panel_handle));
...
TEST_CASE("test co5300 to draw pattern with MIPI interface", "[co5300][draw_pattern]")
{
...
TEST_CASE("test co5300 to draw color bar with MIPI interface", "[co5300][draw_color_bar]")
{
    ESP_LOGI(TAG, "Initialize LCD device");
    test_init_lcd();
...
TEST_CASE("test co5300 to rotate with MIPI interface", "[co5300][mipi-rotate]")
{
