...
#define DISPLAY_BRIDGE_V9_TE_TRANSFER_ROWS   (32U)
...
typedef struct {
    ...
    lv_area_t pending[DISPLAY_BRIDGE_V9_DIFF_RECT_CAPACITY];
} display_bridge_v9_diff_buffer_t;

typedef struct {
    display_bridge_v9_diff_buffer_t buffers[DISPLAY_BRIDGE_V9_DIFF_BUFFER_COUNT];
    uint32_t revision;
} display_bridge_v9_diff_tracker_t;
...
typedef struct esp_lv_adapter_display_bridge_v9 {
    ...
    display_bridge_v9_diff_tracker_t diff_tracker;
    ...
    bool to_physical)
{
    *current_count = 0;
    ...
    for (uint16_t index = 0; index < count; ++index) {
        ...
        if (rotated) {
            display_bridge_v9_diff_area_to_physical(impl, &rect, &rect);
            rect.x1 = LV_MAX(rect.x1, 0);
            rect.y1 = LV_MAX(rect.y1, 0);
        ...
        if (*current_count >= DISPLAY_BRIDGE_V9_DIFF_RECT_CAPACITY) {
            *current_count = 0;
            return false;
        }
                                                ...
                                                uint16_t capacity)
{
    ...
    if (!display_bridge_v9_diff_rect_valid(&overlap)) {
    ...
    uint16_t *repair_count)
{
    ...
    *repair_count = pending_count;
    ...
    display_bridge_v9_diff_coalesce_exact(repair, repair_count);
    return true;
}
                                                   ...
                                                   bool record_flush_post)
{
    ...
    if (ret == ESP_OK) {
        display_bridge_v9_mark_frame_buf_submit(impl);
    }
...
esp_lv_adapter_display_bridge_t *esp_lv_adapter_display_bridge_v9_create(const esp_lv_adapter_display_runtime_config_t *cfg)
{
    ...
    impl->base.dummy_draw_get_free_buf = display_bridge_v9_dummy_draw_get_free_buf;
    impl->base.dummy_draw_flush_buf = display_bridge_v9_dummy_draw_flush_buf;
    ...
    impl->double_wait_sem = NULL;
    impl->frame_buf_submit_pending = false;
    if (display_bridge_v9_configure_double_wait_resource(impl) != ESP_OK) {
        free(impl);
        return NULL;
    }
    ...
    if (!enable) {
        ...
        if (was_enabled && disp &&
            ...
            if (disp->buf_1 && disp->buf_1->data == impl->draw_fb) {
                target = disp->buf_1;
            } else if (disp->buf_2 && disp->buf_2->data == impl->draw_fb) {
                target = disp->buf_2;
            } else if (disp->buf_1 && disp->buf_1->data == impl->disp_fb && disp->buf_2) {
                disp->buf_2->data = impl->draw_fb;
                target = disp->buf_2;
            } else if (disp->buf_2 && disp->buf_2->data == impl->disp_fb && disp->buf_1) {
                disp->buf_1->data = impl->draw_fb;
                target = disp->buf_1;
            } else if (disp->buf_2) {
                disp->buf_2->data = impl->draw_fb;
                target = disp->buf_2;
                                                        ...
                                                        void *frame_buffer)
{
    ...
    if (!rotated && bridge_mode_uses_double_switch_wait(mode)) {
        return display_bridge_v9_dummy_publish_double_wait(impl, frame_buffer);
    }
    if (!rotated && mode == ESP_LV_ADAPTER_TEAR_AVOID_MODE_TRIPLE_FULL) {
        return display_bridge_v9_dummy_publish_triple_pending(impl, frame_buffer);
    }
    return display_bridge_v9_dummy_publish_pipeline(impl, frame_buffer);
}
...
static void display_bridge_v9_wait_for_buf_switch(esp_lv_adapter_display_bridge_v9_t *impl)
{
    ...
    for (;;) {
        ...
        if (!display_bridge_v9_has_pending_submit(impl)) {
            break;
        }
...
display_bridge_v9_handle_frame_buf_complete(esp_lv_adapter_display_bridge_v9_t *impl)
{
    ...
    if (!display_bridge_v9_consume_frame_buf_submit_isr(impl)) {
        return false;
    }
    ...
    if (bridge_mode_uses_rotated_nonpartial_pipeline_release(impl) ||
        ...
        bool double_ready = display_bridge_v9_complete_double_wait_isr(impl);
...
#endif /* CONFIG_SOC_MIPI_DSI_SUPPORTED */
                                                              ...
                                                              void *user_ctx)
{
    ...
    bool buf_switch = display_bridge_v9_handle_frame_buf_complete(impl);
    bool vsync = display_bridge_v9_handle_vsync(impl);
...
static void display_bridge_v9_te_stage_transfer(const esp_lv_adapter_display_bridge_v9_t *impl,
...
static esp_err_t display_bridge_v9_te_stage_prepare_tx(void *user_ctx, size_t transfer_index)
{
    esp_lv_adapter_display_bridge_v9_t *impl = (esp_lv_adapter_display_bridge_v9_t *)user_ctx;
    ...
    size_t data_offset;
    size_t data_size;
    display_bridge_v9_te_stage_transfer(impl, transfer_index, &area, &data_offset, &data_size);
...
static esp_err_t display_bridge_v9_te_stage_start_tx(void *user_ctx, size_t transfer_index)
{
    esp_lv_adapter_display_bridge_v9_t *impl = (esp_lv_adapter_display_bridge_v9_t *)user_ctx;
    ESP_RETURN_ON_FALSE(transfer_index < impl->te_stage_area_count,
    ...
    lv_area_t area;
    size_t data_offset;
    display_bridge_v9_te_stage_transfer(impl, transfer_index, &area, &data_offset, NULL);
...
static void display_bridge_v9_te_stage_done(void *user_ctx)
{
    esp_lv_adapter_display_bridge_v9_t *impl = (esp_lv_adapter_display_bridge_v9_t *)user_ctx;
...
static esp_err_t display_bridge_v9_te_stage_build_full_transfers(esp_lv_adapter_display_bridge_v9_t *impl)
{
    ...
    if (impl->cfg.base.te_sync.bounce_buffer_enabled) {
        rows_per_transfer = impl->te_bounce.count ?
                            ...
                            LV_MIN(DISPLAY_BRIDGE_V9_TE_TRANSFER_ROWS, height);
    }
...
static esp_err_t display_bridge_v9_te_stage_merge_area(esp_lv_adapter_display_bridge_v9_t *impl,
                                                       lv_display_t *disp,
                                                       const lv_area_t *area,
                                                       const uint8_t *color_map)
{
    ...
    const size_t src_stride = disp->buf_act->header.stride;
    const size_t logical_width = (size_t)lv_display_get_horizontal_resolution(disp);
    const size_t physical_width = bridge_h_res(impl);
    const size_t physical_height = bridge_v_res(impl);
    const size_t dst_stride = physical_width * color_bytes;
    const size_t row_bytes = width * color_bytes;
    ESP_RETURN_ON_FALSE(dst_stride * physical_height <= impl->te_stage_capacity, ESP_ERR_INVALID_SIZE, TAG,
                        "Async full TE area exceeds staging buffer");

    if (bridge_rotation(impl) != ESP_LV_ADAPTER_ROTATE_0) {
        ESP_RETURN_ON_FALSE(src_stride == logical_width * color_bytes, ESP_ERR_NOT_SUPPORTED, TAG,
                            "Async TE rotation requires a packed full-screen buffer");
        ...
        if (lv_display_get_color_format(disp) == LV_COLOR_FORMAT_RGB565) {
            lv_area_t physical;
            display_bridge_v9_diff_area_to_physical(impl, area, &physical);
            size_t physical_row_width = (size_t)lv_area_get_width(&physical);
            for (int32_t y = physical.y1; y <= physical.y2; ++y) {
                uint8_t *row = impl->te_stage_buf + ((size_t)y * physical_width + physical.x1) * color_bytes;
                lv_draw_sw_rgb565_swap(row, physical_row_width);
            }
        }
        return ESP_OK;
    }

    const size_t end_offset = (size_t)area->y2 * dst_stride + ((size_t)area->x2 + 1U) * color_bytes;
    ESP_RETURN_ON_FALSE(end_offset <= impl->te_stage_capacity, ESP_ERR_INVALID_SIZE, TAG,
                        "Async full TE area exceeds staging buffer");

    const uint8_t *src = color_map + (size_t)area->y1 * src_stride + (size_t)area->x1 * color_bytes;
    uint8_t *dst = impl->te_stage_buf + (size_t)area->y1 * dst_stride + (size_t)area->x1 * color_bytes;
    for (size_t row = 0; row < height; row++) {
        uint8_t *dst_row = dst + row * dst_stride;
        memcpy(dst_row, src + row * src_stride, row_bytes);
        if (lv_display_get_color_format(disp) == LV_COLOR_FORMAT_RGB565) {
            lv_draw_sw_rgb565_swap(dst_row, width);
        }
    }
    return ESP_OK;
}

static void display_bridge_v9_flush_te_async_full(esp_lv_adapter_display_bridge_v9_t *impl,
                                                  lv_display_t *disp,
                                                  const lv_area_t *area,
                                                  uint8_t *color_map)
{
    ...
    if (impl->te_stage_initialized &&
            display_bridge_v9_te_stage_merge_area(impl, disp, area, color_map) != ESP_OK) {
    ...
    if (!impl->te_stage_initialized) {
        if (display_bridge_v9_te_stage_merge_area(impl, disp, &full_area, color_map) != ESP_OK) {
    ...
    esp_err_t build_ret = display_bridge_v9_te_stage_build_full_transfers(impl);
    if (build_ret != ESP_OK) {
    ...
    const esp_lv_adapter_te_sync_job_t job = {
        ...
        .prepare_tx = impl->te_bounce.count ? display_bridge_v9_te_stage_prepare_tx : NULL,
        .start_tx = display_bridge_v9_te_stage_start_tx,
        .done = display_bridge_v9_te_stage_done,
        .user_ctx = impl,
    };
...
{
    if (esp_lv_adapter_te_sync_is_async(impl->cfg.te_ctx)) {
        display_bridge_v9_flush_te_async_full(impl, disp, area, color_map);
        return;
    }
                                                ...
                                                uint8_t *color_map)
{
    ...
    display_bridge_v9_wait_for_buf_switch(impl);
                                                ...
                                                uint8_t *color_map)
{
    ...
    if (lv_display_flush_is_last(disp)) {
        ...
        esp_err_t ret = display_bridge_v9_blit_full_frame(impl, impl->draw_fb, true);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Blit failed: %s", esp_err_to_name(ret));
        } else {
            display_bridge_pipeline_mark_buf_busy(&impl->pipeline, impl->disp_fb);
                                                ...
                                                uint8_t *color_map)
{
    ...
    display_bridge_v9_get_rotation_geometry(impl, &logical_w, &logical_h, &physical_w, &physical_h);
