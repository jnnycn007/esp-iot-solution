...
/* Peer service memory pool */
static void *peer_svc_mem = NULL;
static struct os_mempool peer_svc_pool = {0};

/* Peer characteristic memory pool */
static void *peer_chr_mem = NULL;
static struct os_mempool peer_chr_pool = {0};

/* Peer descriptor memory pool */
static void *peer_dsc_mem = NULL;
static struct os_mempool peer_dsc_pool = {0};

/* Peer memory pool */
static void *peer_mem = NULL;
static struct os_mempool peer_pool = {0};

/* Peer list */
static SLIST_HEAD(, peer) peers = SLIST_HEAD_INITIALIZER(peers);
...
static uint16_t chr_end_handle(const struct peer_svc *svc, const struct peer_chr *chr);
static int chr_is_empty(const struct peer_svc *svc, const struct peer_chr *chr);
                           ...
                           void *arg)
{
    ...
    assert(peer != NULL && peer->conn_handle == conn_handle);
...
{
    const struct peer_chr *next_chr = NULL;
...
static struct peer_chr *peer_chr_find(const struct peer_svc *svc, uint16_t chr_val_handle, struct peer_chr **out_prev)
{
    ...
    struct peer_chr *chr = (prev == NULL ? SLIST_FIRST(&svc->chrs) : SLIST_NEXT(prev, next));
...
static struct peer_svc *peer_svc_find(struct peer *peer, uint16_t svc_start_handle, struct peer_svc **out_prev)
{
    ...
    struct peer_svc *svc = (prev == NULL ? SLIST_FIRST(&peer->svcs) : SLIST_NEXT(prev, next));
...
{
    const struct peer_svc *svc = NULL;
...
{
    /* Drop stale peer list pointers before freeing the underlying pools. */
    SLIST_INIT(&peers);
    peer_free_mem();
